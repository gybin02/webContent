...
// tests against the current jqLite/jquery implementation if this can be an element
    ...
    angular.module('textAngular.taBind', ['textAngular.factories', 'textAngular.DOM'])
        ...
        .directive('taBind', [
            ...
            function (taSanitize, $timeout, $window, $document, taFixChrome, taBrowserTag, taSelection, taSelectableElements, taApplyCustomRenderers, taOptions, _taBlankTest, $parse, taDOM) {
                                    	...
                                    	var utils = {
                                            ...
                                            var filterWord = function () {
                                                ...
                                                function filterPasteWord( str ) {
                                                    return str.replace(/<\/html>(\s+.*)/gi, "</html>")
                                                    		.replace(/[\t\r\n]+/g,' ')
                                                            .replace( /<!--[\s\S]*?-->/ig, "" )
                                                            //转换图片
...
//                                                                                break;
                                                ...
                                                function filterHtml(text) {
                                                	    var $htmls = $('<div/>').html(text);
                                                	    //如果图片的src是外链，把图片去掉，替换为提示：请上传图片
                                                      	$htmls.find("img").each(function () {
                                                            var $this = $(this);
                                                            var src=$this.attr("src");
                                                            if(src.indexOf("gelonghui.com")<0 && src.indexOf("glhtest")<0){
//                                                               var srcDefault =  "http://res.gelonghui.com/images/pic_placeholder.png";
//                                                               $this.attr("src",srcDefault);
                                                            	$this.after("<br><div style='color:#FF0000'>请上传图片</div><br>");
                                                            	$this.remove();
                                                            }
                                                      	});
                                                        //过滤html title script等
                                                        $htmls.find("title,script,style,link,meta").remove();

                                                        //白名单规则。检测所有非img，a的标签，如果不是这2项，就脱掉这个标签。如<b><a>sdf</a><b>会被脱成<a>sdf</a>
                                                        $htmls.find(":not(a,b,h1,h2,h3,h4,h5,h6,strong,br,table)").each(function () {
                                                            var $this = $(this);
                                                            $this.after($this.html());
                                                            $this.remove();
                                                        });
                                                        //找到所有img，只获取img的src属性，生成一个新的img标签，并放在原img之后。
                                                        $htmls.find("a").each(function () {
                                                            var $this = $(this);
                                                            $this.after('<a href="' + $this.attr("href") + '">' + $this.text() + '</a>');
                                                            //删除原a
                                                            $this.remove();
                                                        });
                                                        $htmls.find("br").each(function () {
                                                            var $this = $(this);
                                                            $this.after('<br>');
                                                            //删除原img
                                                            $this.remove();
                                                        });
                                                        $htmls.find("b,strong,h1,h2,h3,h4,h5,h6").each(function () {
                                                            var $this = $(this);
                                                            $this.after('<' + $this[0].localName + '>' + $this.text() + '</' + $this[0].localName + '>');
                                                            //删除原img
                                                            $this.remove();
                                                        });
                                                        return $htmls.html();
                                                ...
                                                return function ( html ) {
                                                    return (isWordDocument( html ) ? filterPasteWord( html ) : filterHtml($(html)));
                                                };
                                            }();
                                        
                                        text = isHtml? filterWord(text):text.replace(/\r\n/g,"<br/>");
//                                        if (_pasteHandler) text = _pasteHandler(scope, {$html: text}) || text;

                                        if (element.html() == "<br>") {
                                            element.html("<p>" + text + "<br></p>");
                                        } else {
                                ...
                                element.on('paste', scope.events.paste = function (e, eventData) {
                                    ...
                                    if (clipboardData && clipboardData.getData && clipboardData.types.length > 0) {// Webkit - get data from clipboard, put into editdiv, cleanup, then cancel event
                                        ...
                                        processpaste(pastedContent,isHtml);
                                        e.stopPropagation();
                                        e.preventDefault();
                                        return false;
