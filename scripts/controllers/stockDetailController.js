/**
 * 股票详情Created by vincent on 2015/5/7.
 */

GLHApp.controller('StockDetailController', ['$scope', '$location', '$routeParams', 'ApiService', 'CommService', 'DialogService', '$UserService', 'StockService',
    function ($scope, $location, $routeParams, ApiService, CommService, DialogService, $UserService, StockService) {
        ...
        //帖子点赞处理
        $scope.likePost = function (event, postId) {

            ApiService.put(ApiService.getApiUrl().likePost, {postId: postId}, {},
                function (response) {
                    DialogService.toolTip($(event.target), $scope, "点赞成功.");
                },
                function (response) {
                    if (response.statusCode == 403) {
                        $UserService.timeOutTip($(event.target), $scope);
                    } else {
                        DialogService.toolTip($(event.target), $scope, response.message);
                    }
                });
        };
        
        //帖子收藏处理
        $scope.favPost = function (event, postId) {
            ApiService.put(ApiService.getApiUrl().favPost, {postId: postId}, {},
                function (response) {
                    DialogService.toolTip($(event.target), $scope, "收藏成功.");
                },
                function (response) {
                    if (response.statusCode == 403) {
                        $UserService.timeOutTip($(event.target), $scope);
        ...
        $scope.reply = function($event,postId,userId){
	        var _user_editor = angular.element($event.target).parent().parent().parent().next();
