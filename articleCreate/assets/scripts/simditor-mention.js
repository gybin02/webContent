...
SimditorMention = (function(superClass) {
  ...
  SimditorMention.prototype.getItems = function(nurl) {
  	var xurl;
  	if(nurl){
  		xurl=nurl;
  	}else{
  		xurl=this.opts.mention.url;
  	}
    return $.ajax({
      type: 'get',
...
SimditorMention.prototype.filterItem = function() {
    ...
    val = val.replace(String.fromCharCode(1228865), '');
    val = val.replace(String.fromCharCode(1602), '');
    try {
  ...
  SimditorMention.prototype._onKeyUp = function(e) {
    ...
    if (!this.active || $.inArray(e.which, [9, 16, 27, 37, 38, 39, 40]) > -1 || (e.shiftKey && e.which === 50)) {
      return;
    }
